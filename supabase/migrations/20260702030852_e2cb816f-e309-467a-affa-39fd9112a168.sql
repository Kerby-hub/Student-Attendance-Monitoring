...
BEGIN
  ...
  -- GPS accuracy tolerance: cap at 50m to avoid weakening security too much.
  IF _accuracy IS NOT NULL AND _accuracy > 0 THEN
    v_tolerance := LEAST(_accuracy, 50);
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM public.schedule_geofences WHERE schedule_id = v_session.schedule_id) THEN
    ...
    LOOP
      -- Haversine (meters). Latitude first, longitude second.
