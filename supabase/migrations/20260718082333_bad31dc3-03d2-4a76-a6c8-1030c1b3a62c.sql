...
BEGIN
  ...
  -- Late calculation: measure from when the teacher opened the session, not
  -- the scheduled class start time. This handles cases where the teacher
  -- opens the session after the scheduled start (or earlier) — students who
  -- check in within the grace period after opened_at are counted present.
  IF v_session.opened_at IS NOT NULL THEN
    v_late_threshold := v_session.opened_at + make_interval(mins => v_grace_minutes);
    IF v_now > v_late_threshold THEN
      v_status := 'late';
    END IF;
  ELSIF v_schedule.start_time IS NOT NULL THEN
