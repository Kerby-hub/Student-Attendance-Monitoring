...
-- Ensure default late grace is 15 minutes
INSERT INTO public.system_settings(key, value)
VALUES ('late_grace_minutes', to_jsonb(15))
ON CONFLICT (key) DO UPDATE SET value = to_jsonb(15)
  WHERE (public.system_settings.value)::text NOT IN ('5','10','15');

CREATE OR REPLACE FUNCTION public.student_check_in(_qr_token text, _lat double precision, _lng double precision, _accuracy double precision DEFAULT NULL::double precision)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Compute schedule window (based on the session's opened_at date, falling back to today)
  IF v_schedule.start_time IS NOT NULL THEN
    v_class_start := (COALESCE(v_session.opened_at, v_session.created_at, v_now)::date + v_schedule.start_time);
  END IF;
  IF v_schedule.end_time IS NOT NULL THEN
    v_class_end := (COALESCE(v_session.opened_at, v_session.created_at, v_now)::date + v_schedule.end_time);
    v_late_cutoff := v_class_end + make_interval(mins => v_grace_minutes);
  END IF;

  -- Reject check-ins past the late cutoff (end_time + grace)
  IF v_late_cutoff IS NOT NULL AND v_now > v_late_cutoff THEN
    RETURN jsonb_build_object('ok', false, 'code', 'session_ended', 'message', 'Check-in time has ended.');
  END IF;
  ...
  -- Late calculation:
  -- Present if check-in is within [start_time, end_time].
  -- Late if check-in is after end_time but within end_time + grace.
  IF v_class_end IS NOT NULL AND v_now > v_class_end THEN
    v_status := 'late';
  ELSE
    v_status := 'present';
  END IF;
